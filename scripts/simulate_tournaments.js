...
function teamStrength(team) {
  return team.elo * 0.035 + team.recentForm * 0.18 + team.squadRating * 0.18 + team.attack * 0.14 + team.defence * 0.14 + team.worldCupHistory * 0.12 - team.disciplineRisk * 0.05;
}
...
function buildGroupTables(randomized = false) {
  ...
  teams.forEach(team => {
    ...
    tables[team.group][team.name] = { team: team.name, played:0, won:0, drawn:0, lost:0, goalsFor:0, goalsAgainst:0, goalDifference:0, points:0 };
  });

  predictions.filter(match => match.stage === "Group Stage").forEach(match => {
    const home = getTeam(match.home);
    const away = getTeam(match.away);
    const result = randomized ? simulateScore(home, away) : {
    ...
    const a = tables[match.group][match.away];
    h.played++; a.played++;
    h.goalsFor += result.homeGoals; h.goalsAgainst += result.awayGoals;
    a.goalsFor += result.awayGoals; a.goalsAgainst += result.homeGoals;

    if (result.homeGoals > result.awayGoals) { h.won++; a.lost++; h.points += 3; }
    else if (result.homeGoals < result.awayGoals) { a.won++; h.lost++; a.points += 3; }
  ...
  Object.keys(tables).forEach(group => {
    finalTables[group] = Object.values(tables[group])
      ...
      .sort((a,b) => b.points-a.points || b.goalDifference-a.goalDifference || b.goalsFor-a.goalsFor || teamStrength(getTeam(b.team))-teamStrength(getTeam(a.team)));
  });
  return finalTables;
}
...
function getQualifiedTeams(groupTables) {
  ...
  Object.keys(groupTables).sort().forEach(group => {
    qualified.push({ ...groupTables[group][0], qualification: `Winner Group ${group}` });
    qualified.push({ ...groupTables[group][1], qualification: `Runner-up Group ${group}` });
  });
  const thirdPlaced = Object.keys(groupTables).map(group => ({ ...groupTables[group][2], qualification: `Third Group ${group}` }))
    .sort((a,b) => b.points-a.points || b.goalDifference-a.goalDifference || b.goalsFor-a.goalsFor || teamStrength(getTeam(b.team))-teamStrength(getTeam(a.team)))
    .slice(0,8);
...
function simulateKnockout(qualified) {
  ...
  while (round.length > 1) {
    const nextRound = [];
    for (let i = 0; i < round.length; i += 2) nextRound.push(simulateWinner(round[i], round[i+1]));
    round = nextRound;
  }
  return round[0];
}
...
function monteCarlo(runs = 10000) {
  ...
  return Object.entries(winnerCounts).map(([team, wins]) => ({ team, winProbability: Number(((wins/runs)*100).toFixed(2)) }))
...
fs.writeFileSync("data/tournament_simulation.json", JSON.stringify({ generatedAt: new Date().toISOString(), groupTables, qualifiedTeams, winnerProbabilities }, null, 2));

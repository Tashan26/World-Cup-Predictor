...
const knockout = {
  ...
  rounds: [
    { name: "Round of 32", matches: pairTeams(qualified) },
    { name: "Round of 16", matches: Array.from({ length: 8 }, (_, i) => ({ home: `Winner R32-${i * 2 + 1}`, away: `Winner R32-${i * 2 + 2}` })) },
    { name: "Quarter Finals", matches: Array.from({ length: 4 }, (_, i) => ({ home: `Winner R16-${i * 2 + 1}`, away: `Winner R16-${i * 2 + 2}` })) },
    { name: "Semi Finals", matches: [{ home: "Winner QF-1", away: "Winner QF-2" }, { home: "Winner QF-3", away: "Winner QF-4" }] },
    { name: "Final", matches: [{ home: "Winner SF-1", away: "Winner SF-2" }] }
  ]
};
